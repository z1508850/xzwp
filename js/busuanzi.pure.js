...
bszTag = {
        ...
        texts: function (a) {
            this.bszssp.map(function (b) {
                ...
                c &&
                    (c.innerHTML = parseInt(Date.now() * 0.0000005 - 7 * Math.pow(10, 5)) + parseInt(a[b])); // init here
            });
            this.bszssu.map(function (b) {
                var c = document.getElementById("busuanzi_value_" + b);
                c &&
                    (c.innerHTML = parseInt(Date.now() * 0.0000005 - 7.27 * Math.pow(10, 5)) + parseInt(a[b])); // init here
            });
            this.bszspp.map(function (b) {
                var c = document.getElementById("busuanzi_value_" + b);
                c &&
                    (c.innerHTML = parseInt(Date.now() * 0.000000005 - 7.1 * Math.pow(10, 3)) + parseInt(a[b])); // init here
            })
        },
        hides: function () {
